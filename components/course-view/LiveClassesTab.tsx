...
// ---------------------------------------------------------------
// Working version of LiveClassesTab with zoom embed )
...
// ----------------------------------------
// join live class in redirect page 
  ...
  const handleJoin = async (liveClass: ILiveClass) => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        subjectName: getSubjectTitle(liveClass.subjectId)
      });

      window.location.href = `/zoom-meeting.html?${params.toString()}`;
    } catch (err: any) {
      toast.error(err?.message ?? "Failed to join meeting");
      setJoiningId(null);
    }
