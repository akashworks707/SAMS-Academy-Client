...
import {
    CircleDollarSign, GraduationCap, BookOpen,
    CalendarDays, Users, X, TrendingUp,
} from "lucide-react";
...
const colorTokens = {
    ...
    sky: { bg: "bg-sky-50 dark:bg-sky-900/20", icon: "text-sky-600 dark:text-sky-400", sub: "text-sky-500 dark:text-sky-400" },
} as const;
...
export default function TeacherDashboard() {
    ...
    const courseRevenueSummary = res?.data?.courseRevenue?.summary;

    // merge revenue + enrollment chart by period
    ...
    const statCards = stats ? [
        ...
        },
        {
            ...
            sub: `${courseRevenueSummary?.totalRevenue ? fmtBDT(courseRevenueSummary.totalRevenue) : "৳0"} total course revenue`,
            icon: BookOpen,
    ...
    return (
        <div className="space-y-6">
            ...
            {/* ── Chart Row: Salary over time + Enrollment trend ── */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">

                {/* Salary + Classes area chart — 2 cols */}
                            ...
                            <>
                                <ResponsiveContainer width="100%" height={220}>
                                    <AreaChart data={combinedChartData} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
                                        ...
                                        />
                                        <YAxis
                                            yAxisId="cls" orientation="right"
                                            tick={{ fontSize: 11, fill: "#94a3b8" }}
                                            axisLine={false} tickLine={false} allowDecimals={false}
                                        />
                                        <Tooltip content={(p) => <ChartTooltip {...p} granularity={granularity} />} />
                                        <Area
                                            yAxisId="rev" type="monotone" dataKey="revenue" name="Salary"
                                            stroke="#10b981" strokeWidth={2} fill="url(#gSalary)"
                                            dot={{ r: 4, fill: "#10b981", strokeWidth: 0 }} activeDot={{ r: 5 }}
                                        />
                                        <Area
                                            yAxisId="cls" type="monotone" dataKey="classes" name="Classes"
                                            stroke="#6366f1" strokeWidth={2} fill="url(#gClasses)"
                ...
                <Card className="lg:col-span-3 border border-slate-200 dark:border-slate-800 shadow-none bg-white dark:bg-slate-900">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            {courseRevenueSummary && (
                                <span className="text-xs text-slate-500 dark:text-slate-400">
                                    Total: <span className="font-semibold text-emerald-600">{fmtBDT(courseRevenueSummary.totalRevenue)}</span>
                                </span>
                            )}
                        </div>
                    </CardHeader>
                    <CardContent className="px-0 pb-4">
                        {isLoading ? (
                            ...
                            <>
                                {/* Header */}
                                <div className="grid grid-cols-[1.5rem_1fr_3.5rem_5.5rem_5rem] gap-3 px-5 pb-2 border-b border-slate-100 dark:border-slate-800">
                                    <span className="text-[11px] text-slate-400">#</span>
                                    <span className="text-[11px] text-slate-400">Course</span>
                                    <span className="text-[11px] text-slate-400 text-right">Students</span>
                                ...
                                <div className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {courses.map((course, idx) => (
                                        <div
                                            key={course.courseId}
                                            className="grid grid-cols-[1.5rem_1fr_3.5rem_5.5rem_5rem] gap-3 px-5 py-3 items-center hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
                                        >
